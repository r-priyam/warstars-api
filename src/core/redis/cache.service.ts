...
export class CacheService implements Omit<Store, 'delete' | 'clear'> {
    ...
    public async set(key: string, value: string, ttl = 0) {
        ...
        this.logger.debug(
            `${style.green.bold.apply('Cached Data')} | ${style.yellow.bold.apply('KEY:')} ${key} | ${style.red.bold.apply(
                'EXPIRY:'
            )} ${ttl} | ${style.blue.bold.apply('VALUE:')} ${JSON.stringify(value).substring(0, 40)} ...`
        );
        return true;
    }
    ...
    public async get(key: string) {
        ...
        this.logger.debug(
            `${style.magenta.bold.apply('Returning Cached Data')} | ${style.yellow.bold.apply('KEY:')} ${key} | ${style.blue.bold.apply(
                'DATA:'
            )} ${data.substring(0, 40)} ...`
        );
